...
class ShiptimizeShipping {
  ...
  public function shiptimize_order_submited ( $order_id ) {
    ...
    if ( ! empty( $_POST['shiptimizepickup'] ) ) {
      update_post_meta( $order_id, 'shiptimizepickup', sanitize_text_field( $_POST['shiptimizepickup'] ) );
      update_post_meta( $order_id, 'shiptimizepickuplabel', sanitize_text_field( $pickup_label ) );
    }
    ...
    $sql = sprintf( "insert into %sshiptimize (`id`, `status`, `pickup_id`, `carrier_id`, `pickup_label`) VALUES(%d, %d,\"%s\", %d,\"%s\") ",
      $wpdb->prefix,
      $order_id,
  ...
  public function checkout_validation($fields, $errors) {
    ...
    if (!$settings) {
      ...
      return array('error' => 'not a shiptimize method ' . $shipping_method); 
    }
 ...
 */
  function checkout_pickup_field_display_admin_order_meta($order){
    $pickuppoint = get_post_meta( $order->get_id(), 'shiptimizepickup', true ); 
    if ($pickuppoint) {
  ...
  private function get_shiptimize_carrier_from_table_rates( $choosen_method, $order_id = '' ) {
    ...
    if ( !empty( $results ) ) {
      ...
      if( $order_id ){
        update_post_meta($order_id, 'shiptimize_carrier', json_encode($selected_rate));
        update_post_meta($order_id, 'table_rate_plus_rate_id',$results[1]);    
      }
