...
//    Remember that not all items are products 
  ...
  public function set_carrier_cod() {
    ...
    $shiptimize_carriers = json_decode(get_option('shiptimize_carriers'));  
    
    if(count($shiptimize_carriers) == 1) {
      $this->Transporter = $shiptimize_carriers[0]->Id;
    }

    # client has only one carrier 
    if ($this->Transporter) { 
      foreach ($shiptimize_carriers as $carrier) {
  ...
  public function set_carrier(){
    $shiptimize_carrier = get_post_meta( $this->ShopItemId , 'shiptimize_carrier' , true);
    $settings = ''; 
...
//  Regular woo shipping methods you add to zones     
  ...
  public function set_tracking_id($tracking_id, $tracking_url = ''){
    ...
    if ($tracking_id == get_post_meta($this->ShopItemId, 'shiptimize_trackingid', true)) {
    ...
    if ($tracking_url) {
      update_post_meta($this->ShopItemId, 'shiptimize_trackingurl', $tracking_url); 
    }
      ...
      if ( isset($ordermeta->status) && $ordermeta->status == ShiptimizeOrder::$STATUS_EXPORTED_SUCCESSFULLY ) {
        ...
        foreach($post_meta as $m){
          if(stripos($m->meta_key, 'shipping') || stripos($m->meta_key, 'billing')){
            $metastr .= '<br/>'.$m->meta_key.': '.$m->meta_value;  
          } 
        }
